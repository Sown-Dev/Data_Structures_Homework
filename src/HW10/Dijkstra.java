...
public class Dijkstra {
    ...
    public void computeAllEuclideanDistances() {
        ...
        vertexNames.forEach((s, v) -> {
            v.adjacentEdges.forEach(e -> {
                e.distance = computeEuclideanDistance(e.target.x, e.target.y, e.source.x, e.source.y);
            });
        });
    }
    ...
    public void doDijkstra(String s) {
        List<Vertex> availV = new ArrayList<>();

        vertexNames.forEach((str, ver) -> {
        ...
        availV.add(vertexNames.get(s));

        while (!availV.isEmpty()) {
            //System.out.println(availV);
            Vertex v;
            int i=0;
            for( ; i < availV.size();i++){
                if(!availV.get(i).known){
                    v=availV.get(i);
                    //System.out.println("avail     "+availV.get(i));

                    v.adjacentEdges.forEach(e -> {
                        if (e.target.distance > (e.source.distance + e.distance)) {
                            e.target.distance = (e.source.distance + e.distance);
                            e.target.prev=e.source;
                        }
                        if (!availV.contains(e.target) && !e.target.known) {
                            availV.add(e.target);
                        }
                    });
                    v.known=true;
    ...
    public List<Edge> getDijkstraPath(String s, String t) {
        ...
        while(current !=vertexNames.get(s) &&  current !=null){
            Vertex previous =current.prev;
            current.adjacentEdges.forEach(edge->{
